...
${NOTE}
            ...
            <p>The same sheet the app shows under <code>?model</code>, read from the same catalog, so you
                ...
                maximum-length reply would cost; the charge is what the reply actually came to.</p>
            <p>A repository call is priced higher than a plain reply because each one carries the
                repository's file trees and everything read so far as input &mdash; see
                <a href="/docs/git/#cost">repository tasks</a>.</p>
            <div id="priceSheet">
                <p class="hint">Loading the model list&hellip;</p>
            </div>
            <noscript>
                <p class="hint">This sheet is read live from the model catalog, which needs JavaScript.
                    <code>?model</code> in the app lists the same thing.</p>
            </noscript>
