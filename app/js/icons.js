(function () {
    ...
    const STROKE = {
        ...
        archive: '<rect x="2" y="3" width="20" height="5" rx="1"></rect><path d="M4 8v11a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8"></path><path d="M12 11v6"></path><polyline points="9.5 14.5 12 17 14.5 14.5"></polyline>',
        share: '<path d="M12 3v12"></path><polyline points="8 7 12 3 16 7"></polyline><path d="M8 11H6a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-6a2 2 0 0 0-2-2h-2"></path>',
        broom: '<path d="m16 22-1-4"></path><path d="M19 14a1 1 0 0 0 1-1v-1a2 2 0 0 0-2-2h-3a1 1 0 0 1-1-1V4a2 2 0 0 0-4 0v5a1 1 0 0 1-1 1H6a2 2 0 0 0-2 2v1a1 1 0 0 0 1 1"></path><path d="M5 14h14l2 6.8a1 1 0 0 1-1 1.2H4a1 1 0 0 1-1-1.2Z"></path><path d="m8 22 1-4"></path>',
        ...
        eyeOff: '<path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"></path><circle cx="12" cy="12" r="3"></circle><line x1="3" y1="21" x2="21" y2="3"></line>',
    ...
    const BRANDS = {
        ...
        elevenlabs: {
            fill: '#111827',
            box: [4.603, 0, 14.793, 24],
            paths: [
                'M4.6035 0v24h4.9317V0zm9.8613 0v24h4.9317V0z',
            ]
        },
        huggingface: {
            fill: '#FFD21E',
            box: [0, 1.13, 24, 21.738],
            paths: [
        ...
        pixverse: {
            fill: '#DB2777',
            box: [5, 0.001, 15.078, 23.999],
            paths: [
                'M10.998 23.837L5 21.015V5.569c0-.733.32-1.547.84-2.065l1.386-1.387 6.064 19.729c.426 1.377-.987 2.605-2.292 1.991z',
                'M19.36 8.43L12.517.905A2.765 2.765 0 008.517.81L5.84 3.504c-.511.584-.803 1.198-.803 1.93V20.88l13.699-8.219a2.765 2.765 0 00.623-4.23',
            ]
        },
        lightricks: {
            fill: '#0891B2',
    ...
    const BRAND_TINTS = [
        '#B45309', '#047857', '#1D4ED8', '#7E22CE', '#BE123C', '#0E7490',
        '#4D7C0F', '#C2410C', '#6D28D9', '#0F766E', '#A16207', '#9D174D'
    ];

    function brandTint(key) {
        let h = 0;
        for (let i = 0; i < key.length; i++) h = (h * 31 + key.charCodeAt(i)) % 100003;
        return BRAND_TINTS[h % BRAND_TINTS.length];
    }

    function brandInitials(key) {
    ...
    function brand(slug, options) {
        ...
        const key = BRAND_ALIASES[String(slug || '').toLowerCase()]
        ...
        const fill = hit ? hit.fill : brandTint(key);
        let inner;
        if (hit) {
            const inset = 24 - TILE_PAD * 2;
            const scale = inset / Math.max(hit.box[2], hit.box[3]);
            const tx = TILE_PAD + (inset - hit.box[2] * scale) / 2 - hit.box[0] * scale;
            const ty = TILE_PAD + (inset - hit.box[3] * scale) / 2 - hit.box[1] * scale;
            inner = '<g fill="#fff" fill-rule="evenodd" clip-rule="evenodd" transform="translate('
                + round(tx) + ' ' + round(ty) + ') scale(' + round(scale) + ')">'
                + hit.paths.map(d => '<path d="' + d + '"></path>').join('')
                + '</g>';
        } else {
            const text = brandInitials(key);
        ...
        svg.innerHTML = '<rect x="0" y="0" width="24" height="24" rx="6" fill="' + fill + '"></rect>' + inner;
        return svg;
    }
