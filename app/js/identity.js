    ...
    const Identity = {
        ...
        importSecret(input) {
            ...
            if (/^[0-9a-f]{64}$/i.test(text)) {
                sk = unhex(text.toLowerCase());
            } else if (/^nsec1/.test(text)) {
            ...
            this._adopt(sk, NT().getPublicKey(sk), 'local', this._root || NC().pqGenerateRoot());
        },
        ...
        async useExtension() {
            ...
            this._adopt(null, pk.toLowerCase(), 'nip07', this._root || NC().pqGenerateRoot());
        },

        _adopt(sk, pubkey, method, root) {
            this._sk = sk;
            this.pubkey = pubkey;
            this.method = method;
            this._root = root;
        ...
        _deriveKem() {
            ...
            try { this._kem = NC().pqKeypairFromRoot(this._root, 0); } catch (_) { }
        },
